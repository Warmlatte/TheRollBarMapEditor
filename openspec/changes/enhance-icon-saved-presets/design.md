...
Icon tool 目前已具備 IndexedDB icon library、預設 icon seed、顏色 picker、大小/旋轉控制與 map placement。使用者提出的目標畫面要求三個調整：預覽圖示在左、所有使用 ColorPickerGrid 的 color picker 在右；icon size 初始為 1.00x；保留上方預設 icon 列且統一灰色顯示，另新增可保存「目前 icon + 目前色號」的彩色已存圖示列。使用者進一步指定已存圖示需預載內建四個彩色 preset。
...
- 預設 icon picker 中的 icon 全部以 `#7a7a7a` 顯示。
- 已存圖示區塊預設包含 `mountain/#7a7a7a`、`tree/#4a7a3a`、`tower/#7a4a2a`、`skull/#c33232`。
...
### Store default and user saved icon presets in iconStore session state

使用 iconStore 新增 `savedIcons: SavedIconPreset[]` 與 `saveCurrentIcon()`。`savedIcons` 初始值包含四個內建 preset：`mountain/#7a7a7a`、`tree/#4a7a3a`、`tower/#7a4a2a`、`skull/#c33232`。`SavedIconPreset` 僅包含 `svgId` 與 `color`，因為 SVG 原始資料仍由 iconLibraryStore 管理；渲染已存圖示時透過 svgId 反查 library entry。這避免複製 raw SVG，也避免讓 preset 與 library 內容不同步。

替代方案是把彩色 preset 寫進 iconLibraryStore 或 IndexedDB，但這會把「使用者上傳/內建 icon library」與「常用彩色組合」混在同一個持久層，超出本次需求。

### Keep default icon picker gray and add a separate colored saved picker

上方 library picker 繼續作為預設 icon 選擇列，所有 icon 統一以 `#7a7a7a` 顯示；新的「已存圖示」區塊顯示內建彩色 preset 與使用者按下「儲存圖示」後的彩色組合。這符合使用者要求，也避免預設 library icon 的視覺語意和 saved preset 混淆。
...
- The default icon picker SHALL render every library icon with `#7a7a7a`.
- `iconStore.savedIcons` SHALL initialize to the four built-in presets: `{ svgId: "mountain", color: "#7a7a7a" }`, `{ svgId: "tree", color: "#4a7a3a" }`, `{ svgId: "tower", color: "#7a4a2a" }`, `{ svgId: "skull", color: "#c33232" }`.
...
- Unit tests cover iconStore default size, default savedIcons presets, save behavior, duplicate prevention, and no-selection no-op.
- Component tests cover ColorPickerGrid order, default icon picker gray display, IconToolHud save button/section, built-in saved presets, saving a colored icon preset, and restoring a saved preset.
